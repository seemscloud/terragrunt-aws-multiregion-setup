...
locals {
  mgmt_cidr_blocks      = ["10.1.255.240/28"]
  node_cidr_blocks      = ["10.1.10.0/24", "10.1.11.0/24", "10.1.12.0/24"]
  peer_lb_cidr_blocks   = ["10.0.20.0/24", "10.0.21.0/24", "10.0.22.0/24"]
...
inputs = {
  network_acls = {
    ...
    eks_private = {
      vpc_id = dependency.vpc.outputs.vpc_id
      subnet_ids = [
        dependency.subnets.outputs.subnet_ids["private-eu-west-1a"],
        dependency.subnets.outputs.subnet_ids["private-eu-west-1b"],
        dependency.subnets.outputs.subnet_ids["private-eu-west-1c"],
      ]
      ingress = concat(
        [
          {
