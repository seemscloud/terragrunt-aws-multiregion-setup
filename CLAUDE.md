...
- `v-<variable_name>.tf` — one file per `variable` block. **The variable name itself is NOT prefixed.** Example: `v-vpc.tf` contains `variable "vpc" { ... }`, `v-subnets.tf` contains `variable "subnets" { ... }`. Only filenames get the `v-` prefix — variable names stay clean.
- **Resource labels are NOT prefixed.** The second positional argument to `resource` is plain: `resource "aws_vpc" "this"`, `resource "aws_subnet" "this"`, `resource "aws_nat_gateway" "this"`. Use `"this"` for the canonical singleton inside a module. For multiple distinct resources of the same type within one module, use plain descriptive names without any prefix (e.g. `"primary"`, `"replica"`). Only filenames get the `r-` prefix — labels stay clean.
